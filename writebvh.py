...
def editBVH(bvhAbsPath, xy, zy, zx):
    ...
    with open(bvhAbsPath) as bvhFile:
        ...
        for f in range(len(motionSeries)):
            ...
            for i in range(len(mif)):
                if i < 3 and f < 1:
                    # normal.append(mif[i])
                    normal.append(initpos[i])
                    rootMotion.append(initpos[i])
                ...
                if i > 2:
                    rootMotion.append(mif[i])
            # print(rootMotion[0:3])
            newMotionSeries.append(rootMotion)
            rootMotion = []
        # print(len(newMotionSeries))
        # print(motionInFrame)          
        
        dstFilePath = os.path.join("output/" + file.split(".")[0] + "_output" + ".bvh")
        # print(dstFilePath)
